...
python3 main_dino.py \
    ...
    --output_dir vit_tiny_randstainna
"""
...
from seedp.data_models import WSIs
from seedp.datasets import get_slides
...
def get_args_parser():
    ...
    parser.add_argument("--sa_method", default=None)
    parser.add_argument(
        "--patch_size",
        default=16,
        type=int,
...
class DataAugmentationDINO(object):
    def __init__(
        self, global_crops_scale, local_crops_scale, local_crops_number, sa=None
    ):
...
def get_dist_params(slides: WSIs, sampling: float = 1) -> dict:
    ...
    sample_pathces = random.sample(
        slides.patches, math.ceil(len(slides.patches) * sampling)
    )

    for patch in tqdm.tqdm(sample_pathces):
        rgb_image = cv2.imread(patch.path)
...
def aug_transform(sa_method: Literal["randstainna", "stain_vector"]) -> callable:

    if sa_method == "randstainna":
        slides = get_slides(args.data_path)
        print("load slide")
        dist_params = get_dist_params(slides, sampling=0.05)
        print(dist_params)
        config_template = OmegaConf.load("../RandStainNA/CRC_LAB_randomTrue_n0.yaml")
        config_template.update(dist_params)
        OmegaConf.save(config_template, "randstain_config.yaml")

        return RandStainNA(
            yaml_file="randstain_config.yaml",
            std_hyper=-0.3,
            probability=1.0,
            distribution="normal",
            is_train=True,
